...
const productController = {
    ...
    try {
      ...
      if (!req.user) {
        if (!req.session.cartId) {
          return res.render('map/shop/products', { products, page, totalPage, prev, next })
        } else {
          ...
          return res.render('map/shop/products', { products, cart, totalPrice, page, totalPage, prev, next })
        }
      } else {
        ...
        if (!req.session.cartId) {
          ...
          if (!cart) {
            ...
            return res.render('map/shop/products', { products, cart: userCart, totalPrice, page, totalPage, prev, next })
          } else {
  ...
  getProduct: async (req, res, next) => {
    try {
      ...
      if (!req.user) {
        if (!req.session.cartId) {
          return res.render('map/shop/product', { product })
        } else {
          let cart = await Cart.findByPk(req.session.cartId, {
            include: 'cartProducts'
          })
          cart = cart.toJSON()
          const totalPrice = cart.cartProducts.length > 0 ? cart.cartProducts.map(d => d.price * d.CartItem.quantity).reduce((a, b) => a + b) : 0
          return res.render('map/shop/product', { product, cart, totalPrice })
        }
      } else {
        // find if user has cart
        let cart = await Cart.findOne({
        ...
        if (!req.session.cartId) {
          ...
          if (!cart) {
            ...
            userCart = userCart.toJSON()
            const totalPrice = userCart.cartProducts.length > 0 ? userCart.cartProducts.map(d => d.price * d.CartItem.quantity).reduce((a, b) => a + b) : 0
            return res.render('map/shop/product', { product, cart: userCart, totalPrice })
          } else {
            // update cartId to user's cartId
            await CartItem.update(
              { CartId: cart.id },
  ...
  policyPage: (req, res, next) => {
    // 辨認用戶為使用者or管理者  
    return res.render('map/shop/policy')
  },
}
