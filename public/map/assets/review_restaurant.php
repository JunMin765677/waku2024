...
$service_review = $_POST['service_review'];
...
if(trim($name_review) == '') {
	...
	exit();
} else if(trim($service_review ) == '') {
	echo '<div class="error_message">Please rate Service.</div>';
	exit();
} else if(trim($price_review ) == '') {
	echo '<div class="error_message">Please rate Price.</div>';
	exit();
} else if(trim($quality_review ) == '') {
	echo '<div class="error_message">Please rate Quality food.</div>';
	exit();
...
$e_body = "You have been contacted by $name_review $lastname_review with the following Restaurant name review: $restaurant_name." . PHP_EOL . PHP_EOL;
$e_content = "Position rate: $position_review.\nService rate: $service_review.\nRestaurant price rate: $price_review.\nQuality food rate: $quality_review.\n\nReview: $review_text" . PHP_EOL . PHP_EOL;
